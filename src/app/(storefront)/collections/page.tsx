...
export const metadata: Metadata = {
  ...
  description:
    'Explorez les 4 univers joailliers KenDji Luxury : Signature Motifs, Romantic Nature, Urban & Iconic, et Personalized & Cultural. Haute joaillerie en Algérie.',
  alternates: {
        ...
        <Container>
          <div className="max-w-3xl space-y-4">
            <span className="text-[11px] uppercase tracking-[0.3em] text-[#1A1A1A]/60 font-sans flex items-center gap-2">
              ...
              Univers Joailliers Exclusifs
            </span>
            <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-[#1A1A1A]">
              Les Collections
            </h1>
            <p className="text-base sm:text-lg text-[#1A1A1A]/70 font-sans leading-relaxed">
              Chaque collection KenDji raconte une histoire d’exception. De l’inspiration florale à la géométrie urbaine audacieuse, découvrez l’univers qui sublime votre personnalité.
            </p>
          </div>
        </Container>
      </section>
              ...
              >
                ...
                <div className="aspect-[16/10] relative overflow-hidden bg-[#F2F2EF]">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="bg-[#1A1A1A]/90 backdrop-blur-sm text-white text-[10px] uppercase tracking-widest px-3 py-1.5 rounded-none font-medium">
                      {col.productCount} création{col.productCount > 1 ? 's' : ''}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-8 md:p-10 flex-1 flex flex-col justify-between space-y-6">
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-[#1A1A1A]/70 font-sans leading-relaxed line-clamp-3">
                      {col.description}
                    </p>
                  </div>
                  ...
                  <div className="pt-2 border-t border-[#1A1A1A]/5">
                    <Link href={`/collections/${col.slug}`}>
                      <Button
                        variant="outline"
                        className="w-full sm:w-auto border-[#1A1A1A]/20 text-[#1A1A1A] hover:bg-[#1A1A1A] hover:text-white px-8 py-5 text-xs uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 group/btn"
                      >
